...
typedef struct {
    int dias_jupter;
    const char *data_jupter;
    int dias_saturno;
    const char *data_saturno;
} Registro;

int main() {
    Registro dados[50] = {
        {4346, "2032-11-14", 10811, "2050-07-28"},
    ...
    Registro r = dados[n - 1];

    printf("Dias terrestres para Jupiter = %d\n", r.dias_jupter);
    printf("Data terrestre para Jupiter: %s\n", r.data_jupter);
    printf("Dias terrestres para Saturno = %d\n", r.dias_saturno);
    printf("Data terrestre para Saturno: %s\n", r.data_saturno);
